...
PlotMotor(0*DEG_TO_RAD + ang,1*ROV.k1, XYZ + [Sim.Current_X_Y_psi(1:2);0],'1')

% motor 2 (inferior esquerdo)
XYZ = Rot_pose_motor(ang,ROV.dceby,(-ROV.Loa/ft));
PlotMotor(0*DEG_TO_RAD+ang,1*ROV.k1,XYZ + [Sim.Current_X_Y_psi(1:2);0],'2')

% motor 3 (superior esquerdo)
XYZ = Rot_pose_motor(ang,(ROV.dceby),(ROV.Loa/ft));
PlotMotor(0*DEG_TO_RAD+ang,1*ROV.k1,XYZ + [Sim.Current_X_Y_psi(1:2);0],'3')

% motor 4 (inferior direito)
XYZ = Rot_pose_motor(ang,ROV.dcbby,(-ROV.Loa/ft));
PlotMotor(0*DEG_TO_RAD+ang,1*ROV.k1,XYZ + [Sim.Current_X_Y_psi(1:2);0],'4');
